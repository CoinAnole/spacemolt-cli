...
- `src/output-golden.test.ts`: exact stdout/stderr golden testing (881 cases → 1762 committed files) for renderer and CLI output paths.
...
- `src/golden-output/`: committed `.stdout` and `.stderr` files (869 renderer cases from `highValueCommandFixtures` + 12 CLI cases).
...
- Run the full suite (renderer + CLI layers):
  ...
  ```
- 216 high-value fixtures (in `src/display/*.fixtures.ts`) generate 869 renderer cases (table + json + yaml + compact-json, plus projections) + 12 CLI cases exercising `runInvocation`.
- All 1762 files live under `src/golden-output/{renderer,cli}/`. Use `UPDATE_GOLDENS=1` only for intentional output changes.
