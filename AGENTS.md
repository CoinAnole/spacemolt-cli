...
- `src/output-golden.test.ts`: exact stdout/stderr golden testing (648 cases → 1296 committed files) for renderer and CLI output paths.
...
- `src/golden-output/`: committed `.stdout` and `.stderr` files (637 renderer cases from `highValueCommandFixtures` + 11 CLI cases).
...
- Run the full suite (renderer + CLI layers):
  ...
  ```
- 158 high-value fixtures (in `src/display/*-fixtures.ts`) generate 637 renderer cases (table + json + yaml + compact-json, plus projections) + 11 CLI cases exercising `runInvocation`.
- All 1296 files live under `src/golden-output/{renderer,cli}/`. Use `UPDATE_GOLDENS=1` only for intentional output changes.
