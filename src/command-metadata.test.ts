...
describe('command metadata', () => {
  ...
  test('craft help documents queued station-storage production and packages', () => {
    ...
    expect(craftPresetHelp).toContain("'fast' (default)");
    ...
    expect(craftPresetHelp).toContain('public facility');
    expect(craftPresetHelp).toContain('ownership only breaks ties');
    expect(craftPresetHelp).toContain('per-run rental fee');
    expect(craftPresetHelp).toContain("'prefer_own'");
    ...
    expect(craftPresetHelp).not.toContain('Default routing uses that ownership order');
    expect(config?.schema?.preset?.description).toContain('prefer_own');
    expect(config?.schema?.preset?.description).toContain('public facility');
    ...
    expect(help).toContain('preset=prefer_own');
    expect(help).toContain('Omitted preset defaults to fast');
    expect(help).toContain('own idle facility');
    expect(help).toContain("prepays that facility's per-run rental fee");
    ...
    expect(help).not.toContain('Default routing prefers your own facility');
  ...
  test('recycle help documents queued lossy reverse production', () => {
    ...
    expect(recyclePresetHelp).toContain("'fast' (default)");
    ...
    expect(recyclePresetHelp).toContain('ownership only breaks ties');
    expect(recyclePresetHelp).toContain("'workshop' doesn't apply");
    expect(recyclePresetHelp).not.toContain('Default routing uses that ownership order');
    ...
    expect(help).toContain('Omitted preset defaults to fast');
    expect(help).toContain('preset=prefer_own');
    expect(help).toContain("'workshop' doesn't apply");
    expect(help).toContain('workshop does not apply');
    expect(help).not.toContain('Default routing prefers your own recycler');
    expect(help).not.toContain('globally fastest or cheapest');
  ...
  test('inspect help documents the docked-base repair queue and sovereign mint report', () => {
  ...
  test('faction post_mission help documents item_id validation and objective-type rules', () => {
    ...
    expect(postMission?.description).not.toContain('kill_player');
    ...
    expect(postMission?.schema?.objectives?.description).toContain('kill_player');
    expect(postMission?.schema?.objectives?.description).toContain('target_id');
    expect(postMission?.schema?.objectives?.description).toContain('type=bounty');
    expect(postMission?.schema?.objectives?.description).toContain('kill_player objective');
    ...
    expect(COMMANDS.faction_post_mission?.example?.match(/spacemolt faction_post_mission/g)?.length).toBe(1);
    expect(COMMANDS.faction_post_mission?.example).toContain('faction post_mission "Hunt Kestrel"');

    const help = captureHelp('faction post_mission');
    expect(help).toContain('invalid_item');
    expect(help).toContain('deliver_item');
    expect(help).toContain('kill_player');
    expect(help).toContain('Hunt Kestrel');
  ...
  test('notification commands expose exactly the server-emitted type choices', () => {
    ...
    for (const command of ['get_notifications', 'notifications']) {
      ...
      expect(help).toContain('action_error, error, drone_adrift');
  ...
  test('get_action_log advertises explicit event arrays and polling cursors', () => {
    ...
    expect(COMMANDS.get_action_log?.description).toBe(description);
    expect(COMMANDS.get_action_log?.description).toBe(QUERY_REFERENCE_COMMAND_OVERRIDES.get_action_log?.description);
    ...
    expect(eventTypeSchema?.description).toBe(eventTypeDescription);
    expect(eventTypeSchema?.description).toBe(
      QUERY_REFERENCE_COMMAND_OVERRIDES.get_action_log?.schemaExtensions?.event_type?.description,
    );
    expect(eventTypeSchema?.description).toContain('; faction.freight_fee_earned');
