...
export const COMMERCE_FACILITY_COMMAND_OVERRIDES: Record<string, CommandOverride> = {
  ...
  get_active_missions: {
    description:
      'View your active missions and progress. jump, dock, mine, buy, sell, create_buy_order, create_sell_order, sell_wreck, scrap_wreck, survey_system, and complete_mission already re-print this table when the server includes envelope missions on that command. Keep using get_active_missions for kill and crafting, and whenever the last mutation omitted missions. complete_mission prints remaining actives after the rewards block.',
    category: 'Missions',
  ...
  storage_loot: {
    ...
    description:
      'Loot wreck cargo into your hold via spacemolt_storage/loot, or loot a wreck module into cargo as an unfitted item with module_id=. Fit it later at a station with install_mod. Omit wreck_id while towing; distinct from loot_wreck (spacemolt_salvage/loot).',
    ...
    seeAlso: ['loot_wreck', 'get_wrecks', 'storage_view', 'install_mod'],
  ...
  facility_repair: {
    usage: '<facility_id>',
    description:
      "Repair a damaged facility (costs ~30% of original materials and build time). A repair finishes on the next station maintenance cycle; listings report that completion tick, rounded up to the station's maintenance pass, not a tick between passes. A station rebuilds its own faction's damaged facilities automatically, every facility it can afford at once, paid from that faction's storage at the station — it will not stall behind one unpayable bill. NPC-station repair bills for megaproject infrastructure are capped; player-faction stations pay full price. Keep that store stocked and a raided station puts itself back with no command. Use this for a facility the station will not rebuild (your own, or your faction's on somebody else's station), or to jump the queue. Faction-owned repairs draw materials from faction storage and may be initiated by members with facility-management rights; automatic rebuild spends and manual spends are recorded in the faction action log (see get_action_log). A paid Faction Storage already under repair keeps reporting as repairing for the whole job. Damaged-station facility IDs also appear on get_base and inspect of a docked base.",
