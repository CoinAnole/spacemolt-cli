  ...
  parse(argv) {
    ...
    if (!kind || !isIdKind(kind)) {
      return {
        ...
        error: {
          code: 'validation_error',
          message: 'Usage: spacemolt ids <poi|system|item|player|ship|faction> [--search text]',
          customStderr: `${c.red}Error:${c.reset} Usage: spacemolt ids <poi|system|item|player|ship|faction> [--search text]`,
          exitCode: 1,
        },
      };
    }
